...
public:
    ...
    // XOR Encryption (Cross-platform, simple but effective)
    ...
    // AES-256-CBC Encryption (Cross-platform)
    std::vector<uint8_t> aesEncrypt(const std::vector<uint8_t>& data) {
        #ifdef _WIN32
        return aesEncryptWindows(data);
        #else
        return aesEncryptLinux(data);
        #endif
    }

    std::vector<uint8_t> aesDecrypt(const std::vector<uint8_t>& encrypted) {
        #ifdef _WIN32
        return aesDecryptWindows(encrypted);
        #else
        return aesDecryptLinux(encrypted);
        #endif
    }

    // ChaCha20 Encryption (Cross-platform custom implementation)
    ...
    std::string getMethodName(Method method) const {
        switch (method) {
            case Method::XOR: return "XOR";
            case Method::AES: return "AES-256-CBC";
            case Method::CHACHA20: return "ChaCha20";
...
private:
    ...
    #ifdef _WIN32
    // Windows AES implementation using CryptoAPI
    ...
    std::vector<uint8_t> aesDecryptWindows(const std::vector<uint8_t>& encrypted) {
        HCRYPTPROV hProv = 0;
        HCRYPTKEY hKey = 0;
    ...
    #else
    // Linux AES implementation using OpenSSL
    std::vector<uint8_t> aesEncryptLinux(const std::vector<uint8_t>& data) {
    ...
    std::vector<uint8_t> aesDecryptLinux(const std::vector<uint8_t>& encrypted) {
        EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
        if (!ctx) throw std::runtime_error("Failed to create cipher context");
    ...
    // ChaCha20 implementation (cross-platform)
    ...
    void chacha20Init(ChaCha20Context* ctx, const uint8_t* key, const uint8_t* nonce) {
        ...
        // Nonce (96-bit, using first 12 bytes of IV)
        for (int i = 0; i < 3; ++i) {
            ctx->state[13 + i] = 
