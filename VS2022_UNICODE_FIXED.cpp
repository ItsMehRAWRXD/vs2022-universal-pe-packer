...
// Control IDs
constexpr int ID_INPUT_EDIT = 1001;
constexpr int ID_OUTPUT_EDIT = 1002;
...
constexpr int ID_ARCH_COMBO = 1009;
constexpr int ID_CERT_COMBO = 1010;
constexpr int ID_ABOUT_BUTTON = 1011;
...
public:
    static std::string generateRealisticTimestamp() {
        ...
        auto past_time = time_t_val + (static_cast<long long>(daysDist(gen)) * 24 * 60 * 60);
        
        struct tm timeinfo = {};
        gmtime_s(&timeinfo, &past_time);
        char buffer[80] = {};
...
private:
    HWND hInputEdit = nullptr, hOutputEdit = nullptr, hProgressBar = nullptr, hStatusText = nullptr;
    HWND hCompanyCombo = nullptr, hArchCombo = nullptr, hCertCombo = nullptr;
...
public:
    ...
    void CreateControls(HWND hwnd) {
        ...
        hInputEdit = CreateWindow(L"EDIT", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        ...
        hOutputEdit = CreateWindow(L"EDIT", L"", WS_VISIBLE | WS_CHILD | WS_BORDER | ES_AUTOHSCROLL,
        ...
        // Company selection
        CreateWindow(L"STATIC", L"Company Identity:", WS_VISIBLE | WS_CHILD,
                     20, 140, 120, 20, hwnd, NULL, NULL, NULL);
        
        hCompanyCombo = CreateWindow(L"COMBOBOX", L"", WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST | WS_VSCROLL,
        ...
        hArchCombo = CreateWindow(L"COMBOBOX", L"", WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST,
                                  150, 175, 100, 100, hwnd, (HMENU)ID_ARCH_COMBO, NULL, NULL);
        
        // Certificate chain
        CreateWindow(L"STATIC", L"Certificate Chain:", WS_VISIBLE | WS_CHILD,
                     20, 210, 120, 20, hwnd, NULL, NULL, NULL);
        
        hCertCombo = CreateWindow(L"COMBOBOX", L"", WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST | WS_VSCROLL,
                                  150, 210, 200, 200, hwnd, (HMENU)ID_CERT_COMBO, NULL, NULL);
        
        // Create button
        CreateWindow(L"BUTTON", L"Create Stealth Executable", WS_VISIBLE | WS_CHILD | BS_PUSHBUTTON,
        ...
        // Status text
        hStatusText = CreateWindow(L"STATIC", L"Ready to create stealth executable...", 
                                   WS_VISIBLE | WS_CHILD,
...
private:
    void populateComboBoxes() const {
        ...
        std::vector<std::wstring> companies = {
            L"Microsoft Corporation", L"Google LLC", L"Apple Inc.", L"Amazon.com Inc.",
            L"Adobe Systems Inc.", L"Intel Corporation", L"NVIDIA Corporation", L"Oracle Corporation",
            L"Cisco Systems Inc.", L"IBM Corporation", L"Salesforce Inc.", L"VMware Inc.",
            L"Autodesk Inc.", L"Electronic Arts Inc.", L"Symantec Corporation", L"McAfee LLC",
            L"Trend Micro Inc.", L"Kaspersky Lab", L"Avast Software", L"Malwarebytes Corporation"
        };
        ...
        std::vector<std::wstring> architectures = {L"x64", L"x86", L"AnyCPU"};
        ...
        std::vector<std::wstring> certChains = {
            L"Microsoft Code Signing PCA", L"VeriSign Class 3", L"DigiCert SHA2", L"GlobalSign CodeSigning",
            L"Sectigo RSA Code Signing", L"Comodo Code Signing", L"Entrust Code Signing", L"GoDaddy Code Signing",
            L"Thawte Code Signing", L"RapidSSL Code Signing", L"StartCom Code Signing", L"WoSign Code Signing",
            L"COMODO RSA Code Signing", L"DigiCert EV Code Signing", L"VeriSign EV Code Signing", L"GlobalSign EV",
            L"Sectigo EV Code Signing", L"Entrust EV Code Signing", L"QuoVadis EV Code Signing", L"SwissSign EV"
        };
...
private:
        ...
        switch (controlId) {
            ...
            case ID_ABOUT_BUTTON:
                MessageBox(hwnd, 
                    L"Ultimate Stealth Packer v2.0\n\n"
                    L"Features:\n"
                    L"- 10 Advanced Stealth Technologies\n"
                    L"- Completely Benign Output\n"
                    L"- Polymorphic Code Generation\n"
                    L"- VS2022 Compatible\n\n"
    ...
    void BrowseForFile(HWND editControl, bool isInput) {
        ...
        ofn.lpstrFilter = isInput ? L"Executable Files\0*.exe\0All Files\0*.*\0" : 
                                   L"Executable Files\0*.exe\0All Files\0*.*\0";
        ofn.nFilterIndex = 1;
        ofn.Flags = isInput ? OFN_PATHMUSTEXIST | OFN_FILEMUSTEXIST : OFN_OVERWRITEPROMPT;
        
        if ((isInput ? GetOpenFileName(&ofn) : GetSaveFileName(&ofn))) {
            SetWindowText(editControl, fileName);
        }
    }
    ...
    void HandleDropFiles(HDROP hDrop) {
        ...
        if (DragQueryFile(hDrop, 0, fileName, MAX_PATH)) {
            SetWindowText(hInputEdit, fileName);
            
            // Auto-generate output name
            std::wstring inputPath(fileName);
            std::wstring outputPath = inputPath;
            size_t lastDot = outputPath.find_last_of(L'.');
            if (lastDot != std::wstring::npos) {
                outputPath = outputPath.substr(0, lastDot) + L"_stealth" + outputPath.substr(lastDot);
            } else {
                outputPath += L"_stealth.exe";
            }
            SetWindowText(hOutputEdit, outputPath.c_str());
        }
        DragFinish(hDrop);
    }
    ...
    void CreateExecutable() {
        ...
        if (wcslen(inputPath) == 0 || wcslen(outputPath) == 0) {
            MessageBox(NULL, L"Please specify both input and output files.", L"Error", MB_OK | MB_ICONERROR);
            return;
        }
        ...
        wchar_t companyText[256] = {}, certText[256] = {};
        SendMessage(hCompanyCombo, CB_GETLBTEXT, companyIndex, (LPARAM)companyText);
        SendMessage(hCertCombo, CB_GETLBTEXT, certIndex, (LPARAM)certText);
        
        std::string company = wstringToString(std::wstring(companyText));
        std::string arch = getArchitectureName(archIndex);
        std::string cert = wstringToString(std::wstring(certText));
        
        // Update status
        SetWindowText(hStatusText, L"Creating stealth executable...");
        ...
        std::thread([this, inputPath, outputPath, company, arch, cert]() {
            bool success = createUltimateStealthExecutable(
                wstringToString(std::wstring(inputPath)), 
                wstringToString(std::wstring(outputPath)), 
                company, 
                arch, 
                cert
            );
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    while (GetMessage(&msg, NULL, 0, 0)) {
        if (msg.message == WM_USER + 1) {
            ...
            HWND hStatusText = FindWindowEx(msg.hwnd, NULL, L"STATIC", NULL);
            HWND hProgressBar = FindWindowEx(msg.hwnd, NULL, PROGRESS_CLASS, NULL);
            
            if (msg.wParam == 1) {
                SetWindowText(hStatusText, L"Stealth executable created successfully!");
                SendMessage(hProgressBar, PBM_SETPOS, 100, 0);
                MessageBox(msg.hwnd, L"Stealth executable created successfully!", L"Success", MB_OK | MB_ICONINFORMATION);
            } else {
                SetWindowText(hStatusText, L"Failed to create executable. Check debug files for details.");
                SendMessage(hProgressBar, PBM_SETPOS, 0, 0);
                MessageBox(msg.hwnd, L"Failed to create executable. Check debug files for details.", L"Error", MB_OK | MB_ICONERROR);
            }
        }
